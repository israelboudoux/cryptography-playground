package edu.boudoux.merkleTree;
...
public class MerkleTree {
    ...
    public static String rebuildRoot(List<Pair> proofList) {
        assert proofList.size() >= 2;

        Pair pZero = proofList.get(0);
        Pair pOne = proofList.get(1);

        String leftValue;
        String rightValue;
        if(pZero.getDirection() == Direction.LEFT) {
        ...
        for (int i = 2; i < proofList.size(); i++) {
            Pair nodeValue = proofList.get(i);

            if (nodeValue.getDirection() == Direction.LEFT) {
